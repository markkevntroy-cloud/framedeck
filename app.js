...
let S={name:"Untitled Creative Project",items:[],selected:null,z:1,px:0,py:0,h:[],f:[],approved:false,approval:null,quote:[]}, space=false;
...
function snap(){return JSON.stringify({name:S.name,items:S.items})}
...
function load(){try{let x=JSON.parse(localStorage.getItem("framedeck-clean"));if(x)Object.assign(S,x)}catch(e){}$("projectName").value=S.name}
...
if(i.type==="image"||i.type==="video"){b.classList.add("mediaBody");b.innerHTML=i.src?(i.type==="image"?`<img src="${i.src}">`:`<video src="${i.src}" controls muted></video>`):"Upload a file"}
...
$("pitch").onclick=pitch;$("closePitch").onclick=()=>$("pitchModal").hidden=true;$("approve").onclick=approve;$("clientView").onclick=clientView;$("clientView2").onclick=clientView;$("pitchIdea").oninput=()=>{$("pIdea").textContent=$("pitchIdea").value};
